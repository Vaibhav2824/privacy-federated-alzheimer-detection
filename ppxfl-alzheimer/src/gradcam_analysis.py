...
def main():
    ...
    parser.add_argument('--fold', type=int, default=0,
                        help='Explanations are drawn ONLY from this fold\'s held-out test slices')
    ...
    # Load data — restricted to this fold's TEST slices, never training slices
    # (the previous version sampled from all_images.npy, which includes data
    # the model was trained on, making the "explanation" partly a memorisation check).
    ...
    images, labels = all_images[split['test_idx']], all_labels[split['test_idx']]
    print(f"  Fold {args.fold}: explaining {len(images)} held-out test slices "
          f"({len(split['test_subjects'])} subjects)")
    ...
    del checkpoint  # loaded with map_location=device — free the duplicate before the heatmap loop

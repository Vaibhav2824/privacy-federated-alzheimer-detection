...
def main():
    ...
    parser.add_argument('--fold', type=int, default=0,
                        help='Background + explained samples are drawn ONLY from this fold\'s held-out test slices')
    ...
    # Load data — restricted to this fold's TEST slices (background + explained
    # samples both drawn from here), never training slices.
    print("Loading data...")
    ...
    print(f"  Fold {args.fold}: {len(images)} held-out test slices available "
    ...
    del checkpoint  # loaded with map_location=device — free the duplicate before SHAP's many backward passes
